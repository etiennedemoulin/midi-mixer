...
class MixerTracks extends LitElement {
  ...
  _render() {
    return this.middlewares.map(middleware => {
      console.log(middleware.getValues());
    })
  }
  ...
  render() {
    return this.tracks.map(track => {
      ...
      //   console.log(middleware.getValues());
      // })
      const faderMiddleware = track.get('fader').middleware;
      console.log(faderMiddleware);

      // const faderUser = middleware.value;
      const fader = track.get('fader');
      const meter = track.get('meter');
      const mute = track.get('mute');

      // if (track.get('disabled')) {
      //   fader.middleware.range = [[0, 1], [0, 1]];
      //   fader.middleware.value = 0;
      // }

      const faderUser = Math.round(fader.middleware.value * 100) / 100;
      const meterUser = Math.round(meter.middleware.value * 100) / 100;
      ...
      return html`
        <div class="${classMap(classes)}">
          ...
          <div>
            <sc-number
              ?disabled=${track.get('disabled')}
              min=${fader.middleware.range ? fader.middleware.range[1][0] : 0}
              max=${fader.middleware.range ? fader.middleware.range[1][1] : 1}
              .value=${ faderUser }
              @input=${e => {
