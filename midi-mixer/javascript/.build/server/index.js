...
globals.onUpdate(async (updates, oldValues, context) => {
  ...
  if (updates.controllerName) {
    const {
      fader
    } = await import(`./controllers/${updates.controllerName}.js`);
...
filesystem.onUpdate(async updates => {
  ...
  const channels = midiConfig.map(tracks => parseInt(tracks.channel)).sort((a, b) => a < b ? -1 : 1);
...
function onTrackUpdate(newValues, oldValues, context, track) {
  ...
  if (context.source !== 'osc') {
    const bundle = new Bundle([`/track/${track.get('channel')}/fader/user`, track.get('faderUser')], [`/track/${track.get('channel')}/fader/raw`, track.get('faderRaw')], [`/track/${track.get('channel')}/fader/bytes`, track.get('faderBytes')]);
